    ...
    public class Worker : BackgroundService
    {
        ...
        private void InitializeRabbitMq()
        {
            try
            {
                ...
                // Объявляем обменник (exchange) с именем "futures.exchange" и типом "direct".
                _channel.ExchangeDeclare(exchange: "futures.exchange", type: ExchangeType.Direct);
                // Объявляем очередь (queue) с именем "futures.data".
                _channel.QueueDeclare(queue: "futures.data", durable: true, exclusive: false, autoDelete: false, arguments: null);
                // Создаем привязку (binding) между очередью "futures.data" и обменником "futures.exchange", используя ключ маршрутизации "futures.data".
